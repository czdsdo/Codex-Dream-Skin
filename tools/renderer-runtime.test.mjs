...
export async function runRendererRuntimeTest(assetRoot) {
  ...
  assert.match(css, /:is\(main\.main-surface, \.app-shell-main-content-viewport\):has\(\[role="main"\]\)/);
  assert.match(css, /:is\(main\.main-surface, \.app-shell-main-content-viewport\):not\(:has\(\[role="main"\]\)\)/);
