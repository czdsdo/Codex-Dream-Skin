...
((cssText, artDataUrl, rawLayers, themeConfig) => {
  ...
  const LAYERS = rawLayers && typeof rawLayers === "object" ? rawLayers : {};
  ...
  const THEME_VARIABLES = [
    ...
    "--dream-skin-quote", "--dream-skin-art", "--dream-person",
  ];
  ...
  const personUrl = (() => {
    const dataUrl = typeof LAYERS.person === "string" ? LAYERS.person : "";
  ...
  const applyRootState = (root) => {
    ...
    if (personUrl) setStyleProperty(root, "--dream-person", `url("${personUrl}")`);
...
})(__DREAM_SKIN_CSS_JSON__, __DREAM_SKIN_ART_JSON__, __DREAM_SKIN_LAYERS_JSON__, __DREAM_SKIN_THEME_JSON__)
