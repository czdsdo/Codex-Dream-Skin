...
try {
  ...
  if (-not $traySource.Contains('[switch]$SelfTest') -or
    -not $traySource.Contains('[AllowEmptyCollection()][System.Windows.Forms.ToolStripItemCollection]$Items')) {
    throw 'Tray menu self-test or empty item collection compatibility is missing.'
  }
  ...
  if (-not $commonSource.Contains('Use-DreamSkinWindowsPowerShell') -or
    -not $commonSource.Contains('IApplicationActivationManager')) {
    throw 'PowerShell 7 handoff or Store package activation support is missing.'
  }
